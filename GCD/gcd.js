const gcdWikiUrl = 'https://en.wikipedia.org/wiki/Greatest_common_divisor';
const gcdSpareUrl = 'https://brilliant.org/wiki/greatest-common-divisor/';
let webState;
...
function gcdProc(arr){
    if (checkInputValid(arr)) {
        return "The reslut is " + gcdMult(arr);
    } else {
        return "Input is invalid (may contain some non-number elements)";
    }
}
...
function goToWeb() {
    window.location.href = webState ? gcdWikiUrl : gcdSpareUrl;
}