const eeaWikiUrl = 'https://en.wikipedia.org/wiki/Extended_Euclidean_algorithm';
const eeaSpareUrl = 'https://brilliant.org/wiki/extended-euclidean-algorithm/';
let webState;
...
function extendWord(m, n){
    ...
    let lhs = m * x;
    let rhs = n * y;
    let paragraph_1 = "What is more, from the table constructed by applying EEA above, we have also find out that the greatest common divisor of " + m + " and " + n + " is equal to " + theGcd + ", and the certificate of correctness is the last second line (the one I highlighted) " + "and indeed we check that";
...
function goToWeb() {
    window.location.href = webState ? eeaWikiUrl : eeaSpareUrl;
}
